...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a, l1_cache = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        scores, l2_cache = affine_forward(a, self.params['W2'], self.params['b2'])
        ...
        # Backprop
        dscores, dW2, db2 = affine_backward(dL, l2_cache)
        da, dW1, db1 = affine_relu_backward(dscores, l1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(dims)-1):
            ...
            # Don't need batch normalization on final (output) layer
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            # Batch Normalization
            if self.use_batchnorm: 
                x, local_cache = batchnorm_forward(x, self.params['gamma'+str(layer)], 
                                                   self.params['beta'+str(layer)], self.bn_params[i])
                cache.append(local_cache)
            ...
            # Dropout
            if self.use_dropout:
                x, local_cache = dropout_forward(x, self.dropout_param)
                cache.append(local_cache)
        ...
        # Backward Pass
        dx, dW, db = affine_backward(dx, cache.pop()) # Final Affine Layer
